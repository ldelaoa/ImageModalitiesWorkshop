...
def convert_dicom_to_nifty(input_filepaths,patientID,
    ...
    if modality != 'RTSTRUCT' and modality != 'SEG':
    ...
    if modality == 'RTSTRUCT':
        ...
        for rt_file in rtstruct_file:
            ...
            for label, np_mask in masks: #label.lower()
                output_filepath = join(output_folder,patientID,filename+modality.lower() + extension)
...
def get_masks(rtstruct_file, labels, image_position_patient=None, axial_positions=None, pixel_spacing=None, shape=None, dtype=np.int8):
...
def read_structure(rtstruct_file, labels):
    ...
    try:
        for i, roi_seq in enumerate(structure.StructureSetROISequence):
            ...
            for label in labels:            
                if True or roi_seq.ROIName.lower() == label.lower():
